...
public class partC_Eylul {
    ...
    public static void statisticalInformationMenu(Scanner input) {
        ...
        int n = getValidArraySize(input); // Array boyutunu almak için
        double[] arr = new double[n];

        for (int i = 0; i < n; i++) {
            arr[i] = getValidDouble(input, i + 1); // Giriş kontrol metodu
        }
    ...
    public static void distanceBetweenArraysMenu(Scanner input) {
        ...
        int n = getValidArraySize(input); // Array boyutunu almak için tek metod
        ...
        for (int i = 0; i < n; i++) {
            arr1[i] = getValidInteger(input, i + 1); // Giriş kontrol metodu
        }

        System.out.println("Enter elements of the SECOND array (0–9):");
        for (int i = 0; i < n; i++) {
            arr2[i] = getValidInteger(input, i + 1); // Giriş kontrol metodu
        }
    ...
    // Array boyutunu almak için yeniden kullanılabilir metot
    ...
    // Double değer girişi için yeniden kullanılabilir metot
    ...
    // Integer değer girişi için yeniden kullanılabilir metot
    public static int getValidInteger(Scanner input, int index) {
        while (true) {
            ...
            try {
                ...
                else System.out.println("Value must be between 0 and 9!");
            } catch (NumberFormatException e) {
    ...
    // Clear screen method (Console clear)
